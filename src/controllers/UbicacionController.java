...
public class UbicacionController implements Initializable {
    ...
    void addLocation() {
    	...
    	if(fin == true || debut == true) {
    		...
    		try {
				...
				Alert alert = new Alert(AlertType.CONFIRMATION, "Hospital agregado con Éxito", ButtonType.OK);
	    		alert.showAndWait();
			} 
    		catch (SQLException e) {
				e.printStackTrace();
    ...
    void searchLogement() {
    	...
    	if(nb == 0) {
    		Alert alert = new Alert(AlertType.ERROR, "No se encontró ningún Hospital " + txt_searchLogementId.getText() + "", ButtonType.OK );
    		alert.showAndWait();
    	}
    }
    ...
    void searchLocataire() {
    	...
    	if(nbr == 0) {
    		Alert alert = new Alert(AlertType.ERROR, "No se encontró ningún Hospital" + txt_cinSearch.getText() + "", ButtonType.OK);
    		alert.showAndWait();
    	}
    }
